...
- **Experiment tracking / 实验记录**: run directories, epoch history, curves, parameter-group learning rates, elapsed time, environment versions, Git commit metadata, and resumable W&B logging.
...
python train.py --config configs/kaggle_vilt.yaml --device cuda --wandb
```
...
Online W&B tracking for the recommended ViLT run / 推荐 ViLT 训练的在线 W&B 追踪：

```bash
export WANDB_API_KEY=...
pip install -e ".[tracking]"
python train.py --config configs/kaggle_vilt.yaml --wandb
```
