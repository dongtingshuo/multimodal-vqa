...
| Track | Status | Hard accuracy | Internal VQA | Official VQA val (%) |
| --- | --- | ---: | ---: | ---: |
| ViLT seed-42 checkpoint (`v0.3.0`) | Recommended engineering release / 推荐工程版本 | **0.6126** | **0.7101** | **68.42** |
| Staged cross-attention checkpoint (`v0.2.0`) | Archived comparison release / 历史对照版本 | 0.5239 | 0.6233 | Not evaluated |
| Legacy checkpoint (`v0.1.0`) | Archived release / 历史发布 | 0.4775 | Not recorded | Not evaluated |

The ViLT run completed through the configured early-stopping rule at epoch 7, selected epoch 5, and exported predictions for all 214,354 validation questions. The official VQA Evaluation Toolkit reports `68.42` on the VQA v2 validation annotations. This is an official-protocol local validation result, not a test-dev/test-standard leaderboard submission.

ViLT 任务已按配置在 epoch 7 触发早停，最佳权重来自 epoch 5，并已导出全部 214,354 条验证预测。官方 VQA Evaluation Toolkit 在 VQA v2 验证标注上的得分为 `68.42`。这是采用官方协议的本地验证结果，不是 test-dev/test-standard leaderboard 提交成绩。
...
- **Model variants / 模型变体**: `vilt`, `text_only`, `image_only`, `baseline_concat`, `cross_attention`, and `strong_cross_attention` share one model factory and inference interface.
...
- **Experiment tracking / 实验记录**: run directories, epoch history, curves, parameter-group learning rates, elapsed time, environment versions, and Git commit metadata.
...
│   ├── kaggle_vilt.yaml      # Recommended ViLT experiment / 推荐 ViLT 实验配置
...
│   ├── smoke_vilt.yaml       # Offline ViLT-path smoke test / 离线 ViLT 路径检查
...
│   ├── prepare_official_vqa_toolkit.py # Python 3 compatibility / Python 3 兼容准备
│   ├── summarize_experiments.py # Comparison and release gate / 对比与发布门槛
│   ├── summarize_runs.py     # Run-directory leaderboard / run 目录汇总
│   ├── select_vilt_followup.py # Controlled Run-2 selector / 受控第二轮选择器
...
```bash
python scripts/validate_vqa_data.py --root data/vqa --sample-images 20 --strict-full
```
...
python train.py --config configs/kaggle_strong.yaml --device cuda --no-wandb
python train.py --config configs/kaggle_vilt.yaml --device cuda --no-wandb
```
...
The best checkpoint is selected by validation `vqa_score`; `latest.pt` is written every epoch for interruption-safe continuation. The ViLT recipe warms up per optimizer step and then reduces parameter-group learning rates when validation VQA plateaus.

默认按验证集 `vqa_score` 选择最佳权重；每个 epoch 都保存 `latest.pt`，用于中断后继续训练。ViLT 配置按 optimizer step 预热，随后在验证 VQA 停滞时降低各参数组学习率。

Kaggle runs use local artifacts for experiment tracking by default: `training_history.csv`,
`training_curves.png`, `run_metadata.json`, and `run_summary.json`.
...
python train.py --config configs/smoke_vilt.yaml
```
...
评估输出按样本归一化的多标签 loss、硬标签 Top-1 准确率、VQA soft score 和 Top-5 VQA score，并可为完整验证集导出官方提交格式的预测 JSON。内部指标只统计答案在当前词表中的样本。

Run the official VQA v2 validation protocol / 运行官方 VQA v2 验证协议：
...
This repository implements a compact VQA classification pipeline for practical training, controlled evaluation, and local demonstration. Toy reports are not benchmarks. The recommended checkpoint has complete internal validation, full prediction export, and official-toolkit validation scoring. The reported `68.42` is not a test-dev/test-standard leaderboard submission. Full training requires large datasets and suitable GPU resources. It is not a reproduction of large-scale generative vision-language models such as BLIP, Flamingo, or LLaVA.

本工程实现的是一个面向实际训练、受控评估和本地演示的紧凑型 VQA 分类管线。玩具报告不是 benchmark。推荐权重已完成内部全量验证、全量预测导出和官方 toolkit 验证评分；`68.42` 不是 test-dev/test-standard leaderboard 提交成绩。全量训练需要大规模数据集和合适的 GPU 资源。本工程并非 BLIP、Flamingo、LLaVA 等大规模生成式视觉语言模型的复现。
