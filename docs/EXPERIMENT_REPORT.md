...
variable. Online W&B tracking is required, but local/Kaggle artifacts remain the source of record.
...
`scripts/select_vilt_followup.py` 根据已记录历史生成，并且只改变一个变量。W&B 在线监控为必需，
但最终记录仍以本地/Kaggle 产物为准。在 ViLT 达到硬准确率 `0.55`、VQA score `0.65` 且完成
...
- Review W&B charts together with persisted run artifacts after each full training run.
