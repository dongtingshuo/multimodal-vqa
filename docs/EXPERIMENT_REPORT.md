...
| ViLT seed 42 | `configs/kaggle_vilt.yaml`, then `autodl/configs/vilt_resume_4090d.yaml` | `003d7df` continuation workflow; checkpoint metadata lacks source commit | `checkpoints/best.pt` |
...
established the strongest completed cross-attention result: `0.5239` hard accuracy and `0.6233`
VQA score. It is therefore a successful improvement, not a non-improving run.

**结果。** 该运行相比历史权重将硬准确率提高了 `0.0464` 个绝对点，并取得已完成
cross-attention 实验中的最佳结果：硬准确率 `0.5239`、VQA score `0.6233`。因此它是成功改进，
不属于未提升运行。
...
**Decision.** Retain as the archived comparison release. The completed ViLT run supersedes it on
both internal hard accuracy and VQA score.

**决策。** 保留为历史对照发布。完整 ViLT 任务在内部硬准确率和 VQA score 上均已超过该模型。
...
## Completed ViLT Run / 已完成 ViLT 任务
...
Epoch 5 is selected. Epochs 6-7 improved training VQA while validation loss increased and
validation VQA declined, so the configured patience-2 early stop correctly prevented further
overfitting. The final export contains 214,354 unique predictions whose question IDs exactly
match the full validation questions and annotations.

最终选择 epoch 5。epoch 6-7 的训练 VQA 继续提高，但验证 loss 上升且验证 VQA 下降，说明
patience=2 的早停正确阻止了进一步过拟合。最终导出包含 214,354 条唯一预测，question ID 与完整
