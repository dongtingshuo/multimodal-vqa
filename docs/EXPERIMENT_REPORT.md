...
- `strong_cross_attention`: adds gated attention pooling, residual normalization, and a deeper fusion head.
- `vilt`: jointly encodes image patches and question tokens with a pretrained ViLT backbone.

- `text_only`：仅使用问题编码器和答案分类器。
- `image_only`：仅使用图像编码器和答案分类器。
- `baseline_concat`：分类前直接拼接池化后的图像与文本特征。
- `cross_attention`：答案分类前使用双向跨模态注意力。
- `strong_cross_attention`：增加门控注意力池化、残差归一化与更深融合头。
- `vilt`：使用预训练 ViLT backbone 联合编码图像 patch 与问题 token。
...
| Staged `cross_attention` | 12 | **0.5239** | **0.6233** | +0.0464 hard accuracy | Recommended completed candidate |
| `strong_cross_attention` | 22 | 0.4967 | 0.5955 | -0.0272 hard accuracy, -0.0278 VQA | Not promoted |

The active ViLT run is intentionally excluded from this completed-experiment table. It has two
completed epochs and currently reports `0.5891` hard accuracy and `0.6879` internal VQA score,
but the configured run, complete prediction export, and official evaluation are unfinished.

正在进行的 ViLT 任务不计入上述“已完成实验”表。该任务已完成两个 epoch，目前硬准确率为
`0.5891`、内部 VQA score 为 `0.6879`，但配置的完整训练、全部预测导出和官方评估尚未完成。
...
**Decision.** Keep as the recommended completed candidate and comparison baseline. Do not replace
it with a partial run; promotion requires a completed candidate with official evaluation.

**决策。** 保留为当前推荐的已完成候选模型与对比基线。不得使用部分训练结果直接替换；晋升需要
候选任务完整结束并完成官方评估。
...
## Active ViLT Run / 当前 ViLT 任务

Run 1 changes the model family because the completed evidence shows that adding capacity to the
separate ResNet/DistilBERT fusion head increased overfitting without improving validation. It uses
the jointly pretrained `dandelin/vilt-b32-mlm-itm` backbone, a fresh Top-3000 classifier,
unchanged soft VQA targets, no label smoothing, and complete validation data. It does not use a
VQAv2-fine-tuned source checkpoint.

Run 1 更换模型家族，是因为已完成实验表明：继续增加独立 ResNet/DistilBERT 融合头容量会加剧
过拟合，并未改善验证指标。该任务采用联合图文预训练的 `dandelin/vilt-b32-mlm-itm`
...
| --- | ---: | ---: |
| Train loss | 64.6833 | 3.4287 |
| Train hard accuracy | 0.3819 | 0.5977 |
| Train VQA score | 0.4591 | 0.6949 |
| Train Top-5 VQA score | 0.6820 | 0.8813 |
| Validation loss | 3.8765 | **3.4327** |
| Validation hard accuracy | 0.5395 | **0.5891** |
| Validation VQA score | 0.6368 | **0.6879** |
| Validation Top-5 VQA score | 0.8519 | **0.8832** |
| Epoch runtime | 18,830 s | 18,833 s |

Epoch 2 is the current best checkpoint and passes both internal gates. The Kaggle session then
started epoch 3 but ended before validation and checkpoint creation, so only two epochs count as
completed. Both `best.pt` and `latest.pt` record epoch 2; the latter preserves optimizer,
scheduler, scaler, RNG, history, and global step (`27,166`) for continuation from epoch 3.

epoch 2 是当前最佳 checkpoint，并已通过两个内部门槛。随后 Kaggle 会话开始了 epoch 3，
但在验证与 checkpoint 保存前结束，因此只有两个 epoch 计为完成。`best.pt` 与 `latest.pt`
均记录 epoch 2；后者还保存 optimizer、scheduler、scaler、RNG、历史和 global step
（`27,166`），可从 epoch 3 继续。

**Interpretation.** The large epoch-1 training loss reflects the freshly initialized classifier
and early optimization transient; it fell to `3.4287` by epoch 2 while validation improved on all
reported metrics. Two points are not enough to establish convergence or generalization stability.
The observed gain is therefore promising evidence, not a finished benchmark claim.

**结果解释。** epoch 1 较大的训练 loss 来自新初始化分类器与早期优化过渡；到 epoch 2 已降至
`3.4287`，同时所有验证指标均提升。但两个数据点不足以判断收敛和泛化稳定性，因此当前提升属于
有前景的阶段性证据，而不是已完成的 benchmark 结论。
...
The existing staged checkpoint remains the recommended completed candidate until ViLT finishes
Run 1, exports the complete validation predictions, and passes official evaluation. Meeting the
internal gates at epoch 2 is necessary evidence, but it is not sufficient for promotion.

协议最多允许两次全量运行。首轮固定使用 `configs/kaggle_vilt.yaml`；第二轮由
`scripts/select_vilt_followup.py` 根据已记录历史生成，并且只改变一个变量。最终记录以
本地/Kaggle 产物为准，Kaggle 流程不再依赖 W&B 在线监控。在 ViLT 完成 Run 1、导出完整验证预测并
通过官方评估前，继续推荐现有已完成的分阶段 checkpoint。epoch 2 达到内部门槛是必要证据，
但不足以直接晋升。
...
- Resume ViLT Run 1 from epoch 3 and complete its fixed stopping protocol.
- Export all validation predictions and run the official VQA evaluator before promotion.
...
- 从 epoch 3 续训 ViLT Run 1，并完成固定的停止协议。
- 晋升前导出全部验证预测并运行官方 VQA evaluator。
- 增加多次运行的实验汇总和置信区间。
- 增加更细的问题类型体系和人工复核字段。
- 增加注意力可视化。
- 改进答案归一化。
